...
export default function CatalogSidebar({ categories, colorGroups, manufacturers, shades, sizes }: Props) {
  ...
  const activeCount = [activeCat, activeColor, activeMfr, activeShade, activeSize, minPrice, maxPrice].filter(Boolean).length;
  ...
  return (
    <aside className="hidden lg:block w-64 flex-shrink-0 bg-white rounded-xl border border-gray-100 p-4 sticky top-[130px] max-h-[calc(100vh-150px)] overflow-y-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        {activeCount > 0 && (
          <button onClick={reset} className="text-xs text-red-500 hover:text-red-600 font-medium flex items-center gap-1">
        ...
        <ul className="space-y-0.5">
          <li>
            <button
              ...
              className={`w-full text-left flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-colors ${!activeCat ? "bg-sky-50 text-sky-600 font-semibold" : "text-gray-600 hover:bg-gray-50 hover:text-sky-500"}`}
            >
              Все категории
            </button>
          </li>
          {categories.map((cat) => (
            <li key={cat.id}>
              <button
                onClick={() => update("cat", String(cat.id))}
                className={`w-full text-left flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-colors ${activeCat === String(cat.id) ? "bg-sky-50 text-sky-600 font-semibold" : "text-gray-600 hover:bg-gray-50 hover:text-sky-500"}`}
              >
                {cat.name}
              </button>
            </li>
