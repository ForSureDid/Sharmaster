import Image from "next/image";

const benefits = [
  {
    title: "Широкий ассортимент",
    desc: "Более 10000 наименований — латексные, фольгированные, хром, металлик.",
  },
  {
    title: "Высокое качество",
    desc: "Работаем только с проверенными производителями. Гарантия качества каждого товара.",
  },
  {
    title: "Удобный заказ",
    desc: "Оформите заказ через WhatsApp или Instagram в любое удобное время.",
  },
  {
    title: "Индивидуальный подход",
    desc: "Учитываем все пожелания — поможем подобрать шары для любого события.",
  },
];

export default function Benefits() {
  return (
    <section id="services" className="py-12 bg-white border-t border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {benefits.map((b) => (
            <div key={b.title} className="bg-gray-50 rounded-2xl p-5 border border-gray-100 hover:border-sky-200 hover:bg-sky-50/50 transition-all">
              <h3 className="font-bold text-gray-800 mb-2 text-base">{b.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed">{b.desc}</p>
            </div>
          ))}
        </div>

        <div className="mt-6">
          <Image
            src="/donballon-import-banner.png"
            alt="Мы импортируем оригинальную продукцию «ДОН БАЛЛОН» в Республику Казахстан, обеспечивая подлинность товаров, широкий ассортимент и стабильные поставки."
            width={881}
            height={192}
            className="w-full h-auto rounded-2xl"
          />
        </div>
      </div>
    </section>
  );
}
