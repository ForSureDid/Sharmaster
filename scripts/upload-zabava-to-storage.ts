/**
 * Uploads scraped Zabava balloon images to Supabase Storage.
 * Bucket: latex-balloons
 * Key format: Zabava/{transliterated_filename}
 *
 * Run: npx ts-node scripts/upload-zabava-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'scraped-starparty')
...
function storageKey(filepath: string): string {
  return `${FOLDER}/${sanitize(basename(filepath))}`
}
...
async function listBucketKeys(prefix: string): Promise<Set<string>> {
  ...
  while (true) {
    ...
    for (const f of files) {
      if (f.id !== null) keys.add(`${prefix}/${f.name}`)
    }
...
async function main() {
  ...
  console.log(`Found ${allFiles.length} images in scraped-starparty/`)

  console.log('Checking existing keys in Zabava/...')
  const existing = await listBucketKeys(FOLDER)
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_zabava_'))
  const BATCH = 8
