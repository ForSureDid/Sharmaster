/**
 * Uploads scraped Grabo foil images to Supabase Storage.
 * Bucket: foil-Grabo (public)
 * Run: npx tsx scripts/upload-grabo-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'scraped-sharik-grabo')
const BUCKET = 'foil-Grabo'
...
async function main() {
  ...
  console.log(`Found ${allFiles.length} images in scraped-sharik-grabo/`)
  await ensureBucket()
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_grabo_'))
  const BATCH = 8
