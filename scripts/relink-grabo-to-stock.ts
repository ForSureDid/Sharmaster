/**
 * Clears wrong imageUrls for all Grabo StockItems (not from foil-Grabo),
 * then re-links ALL Grabo items from foil-Grabo bucket by article.
 * Run: npx tsx scripts/relink-grabo-to-stock.ts
 */
...
async function main() {
  ...
  console.log('\nLoading all Grabo StockItems...')
  const items = await prisma.stockItem.findMany({
    where: { brand: 'Grabo', article: { not: null } },
  ...
  console.log(`\nLinking ${toLink.length} items from foil-Grabo...`)
