/**
 * Uploads ribbon/bow images to Supabase Storage.
 * Bucket: lenyu-bantyu (public)
 * Source: "All the Files with material here/Ленты и банты"
 * Run: npx tsx scripts/upload-lentu-banty-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'All the Files with material here/Ленты и банты')
...
async function main() {
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_lentu_'))
  const BATCH = 8
