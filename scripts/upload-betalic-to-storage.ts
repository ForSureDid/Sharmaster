/**
 * Uploads scraped Betalic foil balloon images to Supabase Storage.
 * Bucket: foil-Betalic (created if needed, public)
 * Key format: {sanitized_filename}  (flat, no subfolder)
 *
 * Run: npx tsx scripts/upload-betalic-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'scraped-sharik-betalic')
...
function storageKey(filepath: string): string {
  return sanitize(basename(filepath))
}
...
async function main() {
  ...
  console.log(`Found ${allFiles.length} images in scraped-sharik-betalic/`)

  await ensureBucket()

  console.log('Checking existing keys in bucket...')
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_betalic_'))
  const BATCH = 8
