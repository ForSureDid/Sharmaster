/**
 * Uploads scraped Everts balloon images to Supabase Storage.
 * Bucket: latex-Everts (created if needed, public)
 * Key format: {sanitized_filename}  (flat, no subfolder)
 *
 * Run: npx tsx scripts/upload-everts-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'scraped-sharik-everts')
...
async function main() {
  ...
  console.log(`Found ${allFiles.length} images in scraped-sharik-everts/`)
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_everts_'))
  const BATCH = 8
