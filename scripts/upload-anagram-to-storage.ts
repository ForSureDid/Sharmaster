/**
 * Uploads scraped Anagram foil balloon images to Supabase Storage.
 * Bucket: foil-Anagram (created if needed, public)
 * Key format: {sanitized_filename}  (flat, no subfolder)
 *
 * Run: npx tsx scripts/upload-anagram-to-storage.ts
 */
...
const IMAGES_DIR = resolve(process.cwd(), 'scraped-sharik-anagram')
...
async function main() {
  ...
  console.log(`Found ${allFiles.length} images in scraped-sharik-anagram/`)
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_anagram_'))
  const BATCH = 8
