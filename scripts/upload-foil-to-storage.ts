/**
 * Uploads every image from "Воздушные шары из фольги" to Supabase Storage.
 *
 * Bucket:  foil-balloons
 * Key:     {category_slug}/{filename_transliterated}
 *   e.g.  3D_i_Khodyachie_figury/180050P_Shar_3D_24_61_sm_Sfera_Yarkoye_disko_Gradient_1_sht._v_up..jpg
 *
 * Filenames are only transliterated (Cyrillic → Latin), not reformatted.
 * Category folder names are transliterated the same way.
...
// ── Build storage key preserving category/filename structure ──────────────────
...
async function uploadOnce(localPath: string, key: string): Promise<string> {
...
async function uploadWithRetry(localPath: string, key: string): Promise<string> {
...
// ── List all existing keys in bucket (paginated, handles subfolders) ──────────
  ...
  async function listPrefix(p: string) {
    ...
    while (true) {
      ...
      for (const f of files) {
        ...
        if (f.id === null) {
          // This is a "folder" pseudo-entry — recurse into it
...
async function main() {
  ...
  const TEMP_DIR = mkdtempSync(join(tmpdir(), 'sharmaster_foil_'))
  const BATCH = 8
